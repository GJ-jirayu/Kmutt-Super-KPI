...
public class EduqaServiceWSImpl extends PostCommon  implements EduqaService{
	...
	//#####[ START: KPI PERSPECTIVE ]########################################################################################//
		@Override
		public ResultService saveKpiPerspective(KpiPerspectiveModel kpiPerspectiveModel) {
			kpiPerspectiveModel.setServiceName(ServiceConstant.PERSPECTIVE_SAVE);
			ResultService rs = new ResultService();
			try{
				ImakeResultMessage imakeMessage =postMessage(kpiPerspectiveModel,kpiPerspectiveModel.getClass().getName(),"kpiPerspectiveModel",true);
				rs.setError(imakeMessage.getResultMessage().isError());
				rs.setMsgCode(imakeMessage.getResultMessage().getMsgCode());
				rs.setMsgDesc(imakeMessage.getResultMessage().getMsgDesc());
			}catch(Exception e){
		...
		@Override
		public ResultService updateKpiPerspective(KpiPerspectiveModel kpiPerspectiveModel) {
			kpiPerspectiveModel.setServiceName(ServiceConstant.PERSPECTIVE_UPDATE);
			ResultService rs = new ResultService();
			try{
			ImakeResultMessage imakeMessage =postMessage(kpiPerspectiveModel,kpiPerspectiveModel.getClass().getName(),"kpiPerspectiveModel",true);
			rs.setError(imakeMessage.getResultMessage().isError());
		...
		@Override
		public Integer deleteKpiPerspective(KpiPerspectiveModel kpiPerspectiveModel) {
			kpiPerspectiveModel.setServiceName(ServiceConstant.PERSPECTIVE_DELETE);
			ImakeResultMessage imakeMessage =postMessage(kpiPerspectiveModel,kpiPerspectiveModel.getClass().getName(),"kpiPerspectiveModel",true);
			if(imakeMessage.getResultListObj()!=null && imakeMessage.getResultListObj().size()>0)
				return ((KpiPerspectiveModel)imakeMessage.getResultListObj().get(0)).getUpdateRecord();
			else
				return null;
		}

		@Override
		public List<KpiPerspectiveModel> searchKpiPerspective(KpiPerspectiveModel kpiPerspectiveModel) {
			kpiPerspectiveModel.setServiceName(ServiceConstant.PERSPECTIVE_SEARCH);
			ImakeResultMessage imakeMessage =postMessage(kpiPerspectiveModel,kpiPerspectiveModel.getClass().getName(),"kpiPerspectiveModel",true);
			this.resultPage = Integer.parseInt(imakeMessage.getLastpage());
			return imakeMessage.getResultListObj();
		}
		...
		@Override
		public KpiPerspectiveModel findKpiPerspectiveById(Integer KpiPerspectiveId) {
			return null;
		}
	//#####[ END: KPI PERSPECTIVE ]########################################################################################//
